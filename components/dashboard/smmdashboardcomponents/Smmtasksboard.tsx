...
export default function SMMTasksBoard({
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, SMMTask[]>();
    [...filtered]
  ...
  const hasUnresolvedChanges = (task: SMMTask) => task.changes.some((c) => !c.resolved);

  const sendReply = (task: SMMTask, changeId: string) => {
  ...
  const cycleAction = (task: SMMTask): { label: string; next: TaskStatus } | null => {
